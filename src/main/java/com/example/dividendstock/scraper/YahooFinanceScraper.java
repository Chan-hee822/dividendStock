...
import com.example.dividendstock.model.constants.Month;
...
public class YahooFinanceScraper implements Scraper {
	...
	public ScrapedResult scrap(Company company) {
		...
		try {
			...
			Element tbody = tableEle.children().get(1); // thead = 0, tfoot = 2;
...
//				System.out.println(year + "/" + month + "/" + day + " -> " + dividend);
	...
	@Override
	public Company scrapCompanyByTicker(String ticker) {    // ticker 를 이용해서 회사의 메타 정보를 불ㄹ러옴
		...
		try {
			...
			return Company.builder().ticker(ticker)
					.name(title)
					.build();
		} catch (IOException e) {
			e.printStackTrace();
